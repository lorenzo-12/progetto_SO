...
//ia exposed function actually uses the syscall mechanism
int disastrOS_spawnWithPriority(int quantums, void (*f)(void*), void* args) {
  return disastrOS_syscall(DSOS_CALL_SPAWN_W_PRIORITY, quantums, f, args);
}
...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){  
  ...
  //ia add the novel syscall for spawning threads with priority
  syscall_vector[DSOS_CALL_SPAWN_W_PRIORITY]      = internal_spawnWithPriority;
  syscall_numarg[DSOS_CALL_SPAWN_W_PRIORITY]      = 3;
