...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){  
  ...
  syscall_vector[SCRIVI_CODA] 	= scrivi;			//<--- aggiunto da me
  syscall_numarg[SCRIVI_CODA]		= 3;			//<--- aggiunto da me
  
  syscall_vector[LEGGI_CODA] 	= leggi;			//<--- aggiunto da me
  syscall_numarg[LEGGI_CODA]		= 3;			//<--- aggiunto da me
...
int scrivi_coda_syscall (coda* coda, char* s, int size) {		//<-- aggiunto da me
  return disastrOS_syscall(SCRIVI_CODA, coda,s,size);			//<-- aggiunto da me	
}																//<-- aggiunto da me

int leggi_coda_syscall (coda* coda, char* s, int size) {		//<-- aggiunto da me
  return disastrOS_syscall(LEGGI_CODA, coda,s,size);			//<-- aggiunto da me	
}																//<-- aggiunto da me
