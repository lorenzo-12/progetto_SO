...
void childFunction(void* args){
  ...
  //aggiunto da me----->
	...
	//i processi 10 e 11 scrivono su due code diverse
	if(a==11){
		ret=0;
		coda* coda1=apri_coda("coda1");
		while(!ret){
			ret=scrivi_coda_syscall(coda1,"AAAAAAAAAAAAAAAAAAAA",20);
			printf(RED);
		...
		ret=0;
		coda* coda2=apri_coda("coda2");
		while(!ret){
			ret=scrivi_coda_syscall(coda2,"BBBBBBBBBBBBBBBBBBBB",20);
			printf(RED);
			printf("PID:%d  HA SCRITTO:%d\n",disastrOS_getpid(),ret);
			printf(RESET);
		}
	...
	//gli altri leggono
		...
		coda* coda2=apri_coda("coda2");
		while(!ret){
			ret=leggi_coda_syscall(coda2,s,5);
			printf(RED);
			printf("PID:%d   HA LETTO:%d\n",disastrOS_getpid(),ret);
			printf(RESET);
			disastrOS_sleep(10);
	...
	if(a<=5 || a==11) chiudi_coda("coda1");
	else chiudi_coda("coda2");
