...
static const int8_t _transition_table []=
  {
    0,  //0000 [prev_status_encoder_bits|curr_status_encoder_bits]
    1,  //0001
   -1,  //0010
    0,  //0011
   -1,  //0100
...
static inline void ENCODER_update(uint8_t encoder_num) {
  ...
  //   prev_reading = [aabb ccxx] -> shift_factor = -2 -> shifting result = [bbcc xx00] -> output = [0000 xx00]
  //   curr_reading = [ddee ffyy] -> shift_factor =  0 -> shifting result = [ddee ffyy] -> output = [0000 00yy]
  uint8_t idx=
...
ISR(PCINT0_vect) {
  ...
  for (int encoder_num = 0; encoder_num < MAX_ENCODERS; ++encoder_num) {
    ENCODER_update(encoder_num); //ia encoder_counts will be modified in this function
  }
...
int main(void){
  ...
  //ia then the port  
  DDRB &= ~PIN_MASK; //set PIN_MASK pins as input 0xFF
